  ...
  methods: {
    ...
    addToCart(product) {
      ...
      if (quantity <= 0) {
        ...
        this.notification =
          "You can't add more items than available in stock. " +
          this.stockData[product.product_id] +
          " item(s) left.";
        this.notificationType = "danger";
      } else {
    ...
    confirmClearCart() {
      axios
        .post("../includes/api/admin.php", { password: this.adminPassword })
        .then((response) => {
    ...
    updateCashAmount(value) {
      this.cashAmount = parseFloat(
        this.cashAmount.toString() + value.toString()
      );
    ...
    generateInvoice() {
      ...
      doc.text(
        "two-e@goodshot.xyz | goodshot.xyz | 09694201337",
        105,
        22,
        null,
        null,
      ...
      doc.setFont("Helvetica", "normal");
      doc.text(
        `Transaction # ${this.transactionNum}`,
        105,
        y,
        null,
        null,
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.data.success) {
            ...
            this.notification = "Failed to save invoice.";
            this.notificationType = "danger";
          }
        })
        .catch((error) => {
    ...
    confirmPayment() {
      if (this.cashAmount >= this.totalAmount) {
        ...
        const payload = {
          transactionNum: this.transactionNum,
          products: products, // Correct payload structure
        };

        // Axios POST request to update stock
        axios
          .post("../includes/api/sales.php", payload)
          .then((response) => {
            // Handle success
